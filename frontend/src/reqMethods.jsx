...
// CONFIGURATION
// -----------------------------
// 1. Define the Base URL dynamically (Uses Render in Cloud, Localhost on Laptop)
// Remove trailing slashes to avoid double-slash errors
...
async function refreshToken() {
  ...
  try {
    // 2. FIX: Use API_BASE instead of hardcoded localhost
    const response = await fetch(`${API_BASE}/tokenrefresh/`, {
      method: "POST",
      headers: {
...
async function request(method, url, body = null, customHeaders = {}, retry = true) {
  try {
    ...
    // 3. FIX: Construct the Full URL
    // If they passed "http...", leave it alone.
    // If they passed "api/students", stick the API_BASE in front.
    const fullUrl = url.startsWith("http") 
        ? url 
        : `${API_BASE}/${url.replace(/^\/+/, "")}`; // Removes leading slash from url if present
